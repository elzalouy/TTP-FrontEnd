...
const ProjectCounterContainer: FC<Props> = ({ done, client, inProgressProject }) => {

    const notStartedProject = useAppSelector(selectNotStartedProjects);

    const getAllActiveProjects = () => {
        let notStartedArray = notStartedProject?.filter((project) => project.clientId === client._id);
        if (typeof notStartedArray !== "undefined" && typeof inProgressProject === "number") {
    ...
    return (
        <Grid
            ...
            alignItems="center"
        >
            <ProjectNumber title="Active Projects" dataTestId="active-projects-clients" number={projectNumber} />
            <hr
                color="#88888885"
